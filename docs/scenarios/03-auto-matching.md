...
1.  **操作**: 点击顶部标签 `我的资产`。
2.  **视觉重点**: 
    *   在“当前挂单”中，看到这笔成交订单状态已变为 `已成交 (FILLED)`。
    *   在下方的“结算历史”中，看到该记录显示为“● 已清算”，撮合方式为“系统自动”。
3.  **动作**: 点击结算记录最右侧的 `查看 (ExternalLink)` 图标。
4.  **展示**: 弹出一个精美的**电子仓单证明书**，包含清算所公章、唯一 HASH 码、仓库点位等详细合规信息。
5.  **解说**: *“成交不仅是数字的变化。系统自动对接分布式清算子系统，为每一笔交易瞬间生成电子仓单凭证。您可以点击查看，该凭证带有清算所专用章，是后续货物交付和资金清算的权威依据。”*

## 测试验证点
- [ ] 交重叠时，是否立即产生新的 `trade` 对象？
- [ ] 撮合后，买卖双方的 `order` 剩余数量是否正确扣除？
- [ ] 底部成交流是否按时间倒序排列？
- [ ] 点击结算记录是否能弹出包含“清算专用章”的电子仓单模态框？
