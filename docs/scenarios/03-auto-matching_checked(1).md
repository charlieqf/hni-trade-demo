...
2.  **动作**: 
    ...
    *   注意点击前的“保证金预估”显示。
3.  **执行**: 点击 `发布买入报价`。
4.  **结果**: 屏幕右侧立即弹出成交提示 (Toast)，中间行情区的卖单数量减半（100 -> 50）。###与卖单3840报价撮合成功？为什么不核对品种的规格、直径、长度？是有低价优先原则吗？
    *   **验证成功**: "自动撮合成功" 绿色弹窗出现，包含成交价格与吨数。
...
## 测试验证点
- [ ] 价格相交时，是否产生毫秒级的成交反馈？###是的
- [ ] 行情版块：卖方的剩余挂单数量是否实时扣减？###是的
- [ ] 电子仓单：生成的仓单是否包含：唯一 HASH、签署日期及清算专用章？###包含
- [ ] 结算明细：结算历史中是否准确记录了成交价格和时间？###是的
